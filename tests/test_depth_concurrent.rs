...
use faultkit::{clear, inject_global, should_fail_mmap, Fault, Operation};
...
fn serial_lock() -> &'static Mutex<()> { // parking_lot::Mutex
    static LOCK: OnceLock<Mutex<()>> = OnceLock::new();
    LOCK.get_or_init(|| parking_lot::Mutex::new(()))
}

#[test]
fn test_depth_concurrent_hammer() {
    let _guard = serial_lock().lock();
    clear();

    // Inject a failure at call 500
    inject_global(Fault::Mmap { fail_after: 500 }).unwrap();
...
#[test]
fn test_depth_concurrent_probabilistic_hammer() {
    let _guard = serial_lock().lock();
    clear();
