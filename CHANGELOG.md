...
### Fixed
- Fixed relative offset calculation in fault injection: `inject` and `inject_global` now offset `fail_after` and `fail_points` by the current operation call count (`calls`) rather than absolute zero, ensuring sequential injections mid-execution fire at the expected call count and do not leave zombie fail points.
- Made `inject_scoped` and `inject_scoped_global` snapshot acquisition and injection atomic under a single lock/mutation context, preventing state corruption from concurrent injections during guard initialization.
